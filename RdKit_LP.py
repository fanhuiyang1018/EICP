...
def calculate_row(idx, row, mol_descriptors, salt_descriptors, scalers, desc_list):
    """
    Calculate weighted mean descriptors for a single row of data.
...
def process_data(input_file, smiles_dict_1, smiles_dict_2):
    ...
    desc_list = [
        "EState_VSA5", "PEOE_VSA8", "SMR_VSA6", "NumAtomStereoCenters",
        "PEOE_VSA7", "PEOE_VSA1", "FpDensityMorgan3", "fr_lactone"
    ]
    descriptors_df = pd.DataFrame(index=data.index, columns=[f"{desc}_mean" for desc in desc_list])

    # Calculate descriptors for molecules and salts
    ...
    data_with_descriptors = data_with_descriptors.dropna(axis=1, how="any")
...
def main():
    ...
    """
    if len(sys.argv) != 3:
        print("Usage: python RdKit_LP.py CALiSol-23_Dataset_cleaned_data.csv SMILES_dict.txt")
        sys.exit(1)
